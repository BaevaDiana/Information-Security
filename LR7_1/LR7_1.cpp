...
using namespace std;
 ...
 string ruleSetGen() //рандом прав доступа
{
	string M = "RWOX";
...
string commandGen(int n, int m) //кодируем команды
{
	int Sub_n = n;
	int Obj_m = m;
...
void writeGen(int count, int n, int m)//вывод команды в файл согласно условию
{
...
void interpreter() //сам интерпретотатор
{
	int n, m;
...
void main()
{
	...
	srand(time(0));
	int n; // число субъектов n
	int m; // число объектов m 
